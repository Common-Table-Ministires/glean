import Foundation

public struct BookInfo: Identifiable, Hashable, Sendable {
    public let bookOrder: Int
    public let name: String
    public let testament: String
    public let genre: String

    public var id: Int { bookOrder }

    public init(bookOrder: Int, name: String, testament: String, genre: String) {
        self.bookOrder = bookOrder
        self.name = name
        self.testament = testament
        self.genre = genre
    }

    /// Short label for tight grid cells. A hand-maintained map rather than a
    /// truncation rule, since a few names (e.g. Philippians) truncate badly and
    /// this is a fixed, small, well-known list worth doing properly once.
    public var abbreviation: String {
        BookInfo.abbreviations[bookOrder] ?? String(name.prefix(4))
    }

    private static let abbreviations: [Int: String] = [
        1: "Gen", 2: "Exod", 3: "Lev", 4: "Num", 5: "Deut", 6: "Josh", 7: "Judg", 8: "Ruth",
        9: "1Sam", 10: "2Sam", 11: "1Kgs", 12: "2Kgs", 13: "1Chr", 14: "2Chr", 15: "Ezra", 16: "Neh",
        17: "Esth", 18: "Job", 19: "Ps", 20: "Prov", 21: "Eccl", 22: "Song", 23: "Isa", 24: "Jer",
        25: "Lam", 26: "Ezek", 27: "Dan", 28: "Hos", 29: "Joel", 30: "Amos", 31: "Obad", 32: "Jonah",
        33: "Mic", 34: "Nah", 35: "Hab", 36: "Zeph", 37: "Hag", 38: "Zech", 39: "Mal",
        40: "Matt", 41: "Mark", 42: "Luke", 43: "John", 44: "Acts", 45: "Rom", 46: "1Cor", 47: "2Cor",
        48: "Gal", 49: "Eph", 50: "Phil", 51: "Col", 52: "1Th", 53: "2Th", 54: "1Tim", 55: "2Tim",
        56: "Titus", 57: "Phlm", 58: "Heb", 59: "Jas", 60: "1Pet", 61: "2Pet", 62: "1John", 63: "2John",
        64: "3John", 65: "Jude", 66: "Rev",
    ]
}
